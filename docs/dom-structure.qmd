...
```{mermaid}
flowchart LR
    PATH["docs/guide.html"] --> PREFIX["pagePrefix = docs-guide"]
    H["見出し: 日本語の見出し"] --> SLUG["romanize_slug → no_shi"]
    PREFIX --> ID["docs-guide-no_shi"]
    SLUG --> ID
    ID --> ATTR["id=\"docs-guide-no_shi\"<br/>data-anchor-id=\"docs-guide-no_shi\""]
```

実際の HTML は以下のようになります。

```html
<h2 class="anchored" data-anchor-id="docs-guide-no_shi" id="docs-guide-no_shi">
  日本語の見出し
</h2>
```
...
`[h4への移動](#深い階層)` は `idMap` で解決され、`#docs-guide-i` のように書き換わります。
...
```{mermaid}
flowchart LR
    A["docs/index.html の<br/>href=\"guide.html#日本語の見出し\""] --> B["guide.html の idMap"]
    B --> C["href=\"guide.html#docs-guide-no_shi\""]
```
...
- [デモサイト](../demo/index.html)
- [Markdown デモ](../demo/markdown-demo.html)
- [AsciiDoc デモ](../build/adoc-qmd/demo/asciidoc-demo.html)
